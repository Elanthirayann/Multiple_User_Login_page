...
if(isset($_POST["submit"])) {
    ...
    $duplicate = mysqli_query($conn, "SELECT * FROM mfp_emp WHERE username = '$username' OR email = '$email'");
    
    if(mysqli_num_rows($duplicate) > 0) {
        echo "<script>alert('Username or email is already taken');</script>";
    } else {
        if($password == $confirm_password) {
            $query = "INSERT INTO mfp_emp (spv_name, district_name, state_name, admin_name,designation, email, contact_number, username, password) VALUES ('$spv_name', '$district_name', '$state_name', '$admin_name','$designation', '$email', '$contact_number', '$username', '$password')";
